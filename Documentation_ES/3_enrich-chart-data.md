...
2. **Cargar Catálogo de Canciones**: Lee `build_song.db` del Script 2.2 (ya no descarga la base de datos de artistas de GitHub)
...
3. **Si se Encuentra**:
   ...
   - **No se realiza resolución de país/género** (ya fue hecho por Script 2.2)
...
> **Nota**: El sistema de ponderación de colaboraciones (`COUNTRY_TO_CONTINENT`, `GENRE_HIERARCHY`, `resolve_country_and_genre()`) ha sido **movido al Script 2.2**. El Script 3 ahora consume los valores pre-resueltos.
...
# Entrada: Base de datos del catálogo de canciones del Script 2.2 (REEMPLAZA la descarga de DB de artistas)
...
| `SONG_CATALOG_DB_PATH` | Entrada: Catálogo de canciones con país/género pre-resuelto del Script 2.2 |
...
Las siguientes tablas han sido **eliminadas** del Script 3 ya que fueron movidas al Script 2.2:
...
Estas funciones fueron movidas al Script 2.2 donde se ejecutan **una vez por canción única** en el momento de inserción en el catálogo, en lugar de semanalmente por aparición en el chart.
...
- **Base de datos del catálogo de canciones del Script 2.2** (`charts_archive/2_2.build-song-catalog/build_song.db`) - **NUEVO REQUISITO**
...
```bash
# Verificar que el catálogo del Script 2.2 existe
...
| :------------------------------------ | :---------------------------- | :--------------------------------------------------- |
| `Song catalog not found`              | Script 2.2 no se ha ejecutado | Ejecutar `2_2.build_song_catalog.py` primero         |
...
| `NULL id in enriched output`          | Canción no está en catálogo   | Ejecutar Script 2.2 para agregar canciones faltantes |
...
1. **Ejecutar Script 2.2** para construir el catálogo de canciones con país/género resuelto
2. **Actualizar su workflow** para asegurar que el Script 2.2 se ejecute antes del Script 3
3. **Eliminar** cualquier modificación personalizada a `COUNTRY_TO_CONTINENT` o `GENRE_HIERARCHY` del Script 3
4. **Agregar** esas personalizaciones al Script 2.2 en su lugar
...
- **Fuentes de Datos**:
  ...
  - Catálogo de canciones del Script 2.2
