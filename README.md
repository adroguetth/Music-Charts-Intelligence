...
A fully automated, end-to-end pipeline that downloads YouTube's weekly music charts, enriches every artist with geographic and genre metadata, augments each chart entry with deep YouTube video metadata, and generates AI-powered Jupyter notebooks — all running on GitHub Actions, zero manual intervention required.
...
| **4_1.weekly_charts_notebook_generator.py** | Generates bilingual Jupyter notebooks with AI insights       | [README](https://github.com/adroguetth/Music-Charts-Intelligence/blob/main/Documentation_EN/4_1.weekly_charts_notebook_generator.md) · PDF | [README](https://github.com/adroguetth/Music-Charts-Intelligence/blob/main/Documentation_ES/4_1.weekly_charts_notebook_generator.md) · PDF |
...
The pipeline processes data in four distinct stages, each building on the previous one's output:
...
| Script 4 | Script 3's enriched DB             | `Notebook_EN/` + `Notebook_ES/`       | 2 notebooks/week          |

------

## ⚙️ Automation Schedule

All four scripts are orchestrated by GitHub Actions and run automatically every Monday:
...
| `4_generate-weekly-notebooks.yml` | Monday 15:00   | Cron + after Script 3 | 20 min  |
...
Scripts 1 and 2 require no API keys. Script 3 works without a key but falls back to slower methods (Selenium, yt-dlp). Script 4 works without DeepSeek but shows placeholder text instead of AI insights.
...
Each script can be run independently. Scripts 2, 3, and 4 depend on previous scripts' outputs existing.
