...
# ============================================================================
# DEEPSEEK CLIENT AND CACHE
# ============================================================================
_DEEPSEEK_CLIENT = None
_DEEPSEEK_CACHE = {}  # Cache for DeepSeek results: {artist: (country, genre)}
...
# ============================================================================
# DEEPSEEK API INTEGRATION (FALLBACK ONLY)
# ============================================================================

def _get_deepseek_client():
    """
    Lazy initialization of DeepSeek client using OpenAI-compatible interface.
...
def search_deepseek_fallback(artist: str, context_country: Optional[str] = None) -> Tuple[Optional[str], Optional[str], str]:
    """
    Use DeepSeek API as a fallback to get country and/or genre.
    Only called when other sources (MusicBrainz, Wikidata, Wikipedia) return nothing.

    Args:
        artist: Artist name to search for
        context_country: Optional country already known (to help DeepSeek)
    ...
    # Rate limiting: wait 0.5 seconds before making the request
    time.sleep(0.5)

    # Build prompt: include context country if known
    if context_country:
        prompt = f"""
...
def search_artist_genre(artist: str, country: Optional[str] = None) -> Tuple[Optional[str], str]:
    ...
    4. DeepSeek API (fallback only when no candidates found)
    ...
    # --- DEEPSEEK FALLBACK (only if still no candidates) ---
    if not all_candidates:
        logger.debug(f"  🔍 Using DeepSeek fallback for genre: {artist} (country: {country})")
        deepseek_country, deepseek_genre, source = search_deepseek_fallback(artist, context_country=country)
        if deepseek_genre:
            # Try to normalize the genre
            ...
            if macro:
                ...
                # If can't map, return the raw genre as a fallback macro
...
def search_country(artist: str) -> Tuple[Optional[str], str]:
    ...
    deepseek_country, deepseek_genre, source = search_deepseek_fallback(artist)
    if deepseek_country:
        return deepseek_country, f"DeepSeek API"
